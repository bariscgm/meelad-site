...
export const getGroupMembers = (program, winner, candidates) => {
  ...
  const cleanWinnerName = winner.name?.trim().toLowerCase();
  
  // If the winner refers to a candidate that is explicitly a group (has members)
  const groupCandidate = candidates.find(c => c.name?.trim().toLowerCase() === cleanWinnerName && c.isGroup);
  ...
  return candidates.filter(c => {
    ...
    const progId = program._id?.toString() || program.id?.toString();
    ...
    const cleanAssignedGroup = assignedGroup?.trim().toLowerCase();
    
    if (cleanAssignedGroup && cleanAssignedGroup === cleanWinnerName && teamsMatch) return true;
    
    // 3. Match by Team and Name (if winner.name is the team name and candidate is in that team and program)
    const wTeamName = winner.team?.name?.trim().toLowerCase();
    
    if (cleanWinnerName === wTeamName && teamsMatch && c.programs?.includes(progName)) return true;
